...
const MasterLayout: FC<IMasterDataProps> = ({ children }) => {
  ...
  const isLanding = router.pathname.split('/').slice(1).length === 1 && !router.pathname.split('/').slice(1)[0];
  ...
  useEffect(() => {
    document.documentElement.setAttribute('data-theme', darkMode && !isLanding ? 'dark' : 'light');
  }, [darkMode, isLanding]);
  if (!width) return null;
  return (
    <div className={styleCombine(styles.master_style, isLanding ? styles.landing : '')}>
